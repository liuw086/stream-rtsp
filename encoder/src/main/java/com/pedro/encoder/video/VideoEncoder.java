...
public class VideoEncoder extends BaseEncoder implements GetCameraData {
    ...
    public boolean prepareVideoEncoder(int width, int height, int fps, int bitRate, int rotation,
        ...
        try {
            ...
            MediaCodecInfo.CodecCapabilities codecCapabilities = codec.getCodecInfo().getCapabilitiesForType(type);
            int maxProfile = 0;
            int maxLevel = 0;
            for(MediaCodecInfo.CodecProfileLevel profileLevel: codecCapabilities.profileLevels){
                ...
                maxProfile = profileLevel.profile > maxProfile ? profileLevel.profile : maxProfile;
                maxLevel = profileLevel.level > maxLevel ? profileLevel.level : maxLevel;
            }
//            this.avcProfile = maxProfile;
//            this.avcProfileLevel = maxLevel;
    ...
    public void requestKeyframe() {
        Log.d(TAG, "requestKeyframe curforceKey=" + forceKey + "; spsPpsSetted="+spsPpsSetted);
        if (isRunning()) {
            if (spsPpsSetted) {
                Bundle bundle = new Bundle();
                ...
                try {
                    ...
                    Log.d(TAG, "requestKeyframe has send");
                } catch (IllegalStateException e) {
                    Log.e(TAG, "encoder need be running", e);
                }
            } else {
                ...
                forceKey = true;
                Log.d(TAG, "requestKeyframe setforceKey=" + forceKey);
            }
        }
    }
    ...
    protected MediaCodecInfo chooseEncoder(String mime) {
        ...
        Log.i(TAG, mediaCodecInfoList.size() + " encoders found force="+force);
        for (MediaCodecInfo mci : mediaCodecInfoList) {
            ...
            for(MediaCodecInfo.CodecProfileLevel profileLevel: codecCapabilities.profileLevels){
                Log.d(TAG, "get Encoder " + mci.getName() +" profile="+profileLevel.profile+" Level= "+profileLevel.level);
            }
            for(int color : codecCapabilities.colorFormats){
                Log.d(TAG, "get Encoder " + mci.getName() +"Color supported: " + color);
            }
        }
        for (MediaCodecInfo mci : mediaCodecInfoList) {
