            ...
            if (isEnableLogs) {
                if (rtpFrame.isVideoFrame()) {
                    ...
                    if (videoCount % 1000 == 0) {
                        ...
                        var bw = sumLen.toFloat() / useTime.toFloat()
                        Log.d(
                            TAG,
                            "wrote packet: ${(if (rtpFrame.isVideoFrame()) "Video" else "Audio")}," +
                                    " sumLen=${sumLen} , count=$videoCount, use=$useTime ms bw=$bw KB/s"
                        )
                    ...
                    audioCount++
                    if (audioCount % 2000 == 0) {
                        Log.d(
                            TAG,
                            "wrote packet: ${(if (rtpFrame.isVideoFrame()) "Video" else "Audio")}, size: ${len + tcpHeader.size} / $audioCount"
                        )
                    }
                }
