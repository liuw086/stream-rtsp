...
//      Log.i(TAG, "Video keyframe[${ts}] send $naluLength ;")
//    }
    ...
    if (type == RtpConstants.IDR || bufferInfo.flags == MediaCodec.BUFFER_FLAG_KEY_FRAME) {
      stapA?.let {
        ...
        updateSeq(buffer)
        val rtpFrame = RtpFrame(buffer, rtpTs, it.size + RtpConstants.RTP_HEADER_LENGTH,
          rtpPort, rtcpPort, channelIdentifier, bufferInfo.flags == MediaCodec.BUFFER_FLAG_KEY_FRAME)
    ...
    if (sendKeyFrame) {
      ...
      if (naluLength <= maxPacketSize - RtpConstants.RTP_HEADER_LENGTH - 1) {
        ...
        markPacket(buffer) //mark end frame
        updateSeq(buffer)
        val rtpFrame = RtpFrame(buffer, rtpTs, buffer.size, rtpPort, rtcpPort, channelIdentifier, bufferInfo.flags == MediaCodec.BUFFER_FLAG_KEY_FRAME)
        videoPacketCallback.onVideoFrameCreated(rtpFrame)
      } else {
        // Set FU-A header
