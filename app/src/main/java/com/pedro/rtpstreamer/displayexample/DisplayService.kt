...
class DisplayService : Service() {
  ...
  private fun prepareStreamRtp() {
    ...
    if (endpoint!!.startsWith("rtmp")) {
      displayBase = RtmpDisplay(baseContext, false, connectCheckerRtp)
      displayBase?.setIntentResult(resultCode!!, data)
    } else {
      displayBase = RtspDisplay(baseContext, false, connectCheckerRtp)
      (displayBase as RtspDisplay)?.setVideoCodec(VideoCodec.H264)
      displayBase?.setIntentResult(resultCode!!, data)
    }
  }
