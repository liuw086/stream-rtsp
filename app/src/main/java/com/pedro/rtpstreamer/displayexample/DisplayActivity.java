...
public class DisplayActivity extends AppCompatActivity
    ...
    private int DEFAULT_RTSP_PORT = 1935;
...
//    etUrl.setHint(R.string.hint_rtmp);
        etUrl.setText(displayIpAddress());

        getInstance();

        if (DisplayServerService.Companion.isStreaming()) {
    ...
    private String displayIpAddress() {
        ...
        if (info != null && info.getIpAddress() > 0) {
            int i = info.getIpAddress(); // "127.0.0.1"; //
    ...
    private void getInstance() {
        DisplayServerService.Companion.init(this);
    }
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                Toast.makeText(DisplayActivity.this, "Connection failed. " + reason, Toast.LENGTH_SHORT)
                        .show();
                stopNotification();
                stopService(new Intent(DisplayActivity.this, DisplayServerService.class));
                button.setText(R.string.start_button);
            }
        });
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (data != null && (requestCode == REQUEST_CODE_STREAM
            ...
            initNotification();
            DisplayServerService.Companion.setData(resultCode, data);
            Intent intent = new Intent(this, DisplayServerService.class);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.b_start_stop:
                if (!DisplayServerService.Companion.isStreaming()) {
                    button.setText(R.string.stop_button);
                    startActivityForResult(DisplayServerService.Companion.sendIntent(), REQUEST_CODE_STREAM);
                } else {
                    button.setText(R.string.start_button);
                    stopService(new Intent(DisplayActivity.this, DisplayServerService.class));
                }
                if (!DisplayServerService.Companion.isStreaming() && !DisplayServerService.Companion.isRecording()) {
                    stopNotification();
                }
                break;
            default:
